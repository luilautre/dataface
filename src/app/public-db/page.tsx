        ...
        <div className="bg-white rounded-xl shadow p-6 space-y-6">
          <div>
            ...
            <p className="text-sm text-facebook-muted mt-2">
              Créez votre propre base liée à la base publique ou une base vide.
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium text-facebook-text mb-1">Nom de la base</label>
            <input
              type="text"
              value={newName}
          ...
          <div className="grid gap-3">
            <button onClick={createLinked} className="w-full bg-facebook-blue text-white py-3 rounded-lg font-medium hover:bg-facebook-dark">
              🔗 Créer une base liée à la publique
            </button>
            <button onClick={createEmpty} className="w-full bg-gray-200 text-facebook-text py-3 rounded-lg font-medium hover:bg-gray-300">
              Créer une base vide
            </button>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-facebook-muted mt-1">
              {db.linkedToPublic ? (
                <>
                  Liée à la <Link href="/public-db" className="text-facebook-blue underline">base publique</Link> · Créée le {new Date(db.createdAt).toLocaleDateString("fr-FR")}
                </>
              ) : (
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            />
            <button onClick={addRow} className="bg-facebook-blue text-white px-3 py-2 rounded-lg text-sm font-medium hover:bg-facebook-dark">
              + Ligne
            </button>
            <button
              onClick={() => {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-facebook-gray border-b border-gray-200">
                {db.columns.map((col) => (
                  <th key={col.key} className="px-3 py-3 text-left font-semibold text-facebook-text whitespace-nowrap">
                    {col.label}
                    {(col.key === "notes" || col.key === "status") && <span className="ml-1 text-xs text-facebook-blue">(perso)</span>}
                  </th>
                ))}
              </tr>
            </thead>
                    ...
                    {db.columns.map((col) => (
                      <td key={col.key} className="px-2 py-1">
                        {col.key === "notes" || col.key === "status" || !db.linkedToPublic ? (
                          ...
                          <span className="px-2 py-1.5 block text-facebook-text">{String(row[col.key] ?? "")}</span>
                        )}
                      </td>
                    ))}
                  </tr>
