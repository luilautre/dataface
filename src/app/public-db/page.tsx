    ...
    <div className="py-4 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-facebook-muted mt-1">
              Tableau partagé type Excel – utilisable par tous. Créez votre propre version liée dans{" "}
              <Link href="/my-db" className="text-facebook-blue underline">
                Ma base
              </Link>
              .
            </p>
          ...
          <div className="flex gap-2">
            ...
            <Link
              href="/my-db"
              className="bg-facebook-blue text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-facebook-dark whitespace-nowrap"
            >
              Créer ma base →
            </Link>
          </div>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr
                  key={String(row.id)}
                  className={`border-b border-gray-100 hover:bg-facebook-light ${
        ...
        <div className="px-4 py-3 border-t border-gray-100 text-sm text-facebook-muted">
          {filtered.length} ligne(s) affichée(s) · Structure partagée – multi-usages (CRM, contacts, suivi…)
        </div>
      </div>
    </div>
  );
