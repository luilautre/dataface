    ...
    <header className="fixed top-0 left-0 right-0 bg-white shadow-md z-50 border-b border-gray-200">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={isAllowed ? item.href : "/login"}
        ...
        <div className="flex items-center gap-2">
          {user ? (
            <>
              <Link href="/admin" className="text-sm text-facebook-blue hover:underline">
                Admin
              </Link>
              <SignOutButton />
            </>
            ...
            <>
              <Link href="/login" className="text-sm text-facebook-blue hover:underline">
                Connexion
              </Link>
              <Link href="/register" className="bg-facebook-blue text-white px-3 py-2 rounded-lg text-sm font-medium hover:bg-facebook-dark">
                S’inscrire
              </Link>
            </>
          )}
